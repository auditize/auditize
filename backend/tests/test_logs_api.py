...
async def test_create_log_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_post(
        f"/repos/{repo.id}/logs", json=PreparedLog.prepare_data()
    )


async def test_create_log_all_fields(log_write_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_create_log_missing_event_name(log_write_client: HttpTestHelper, repo: PreparedRepo):
    data = PreparedLog.prepare_data()
    del data["event"]["name"]
    await log_write_client.assert_post(
...
async def test_create_log_invalid_rich_tag(log_write_client: HttpTestHelper, repo: PreparedRepo):
    invalid_tags = (
...
async def test_add_attachment_text_and_minimal_fields(log_write_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_add_attachment_binary_and_all_fields(log_write_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
...
async def test_add_attachment_forbidden(log_write_client: HttpTestHelper, no_permission_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
    await no_permission_client.assert_forbidden_post(
        f"/repos/{repo.id}/logs/{log.id}/attachments",
        files={"file": ("test.txt", "test data")},
...
async def test_get_log_minimal_fields(log_read_client: HttpTestHelper, log_write_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
...
async def test_get_log_all_fields(log_read_client: HttpTestHelper, log_write_client: HttpTestHelper, repo: PreparedRepo):
    data = PreparedLog.prepare_data({
        "source": {
            "ip": "1.1.1.1",
            "user_agent": "Mozilla/5.0"
...
async def test_get_log_forbidden(log_write_client: HttpTestHelper, no_permission_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
...
async def test_get_log_attachment_text_and_minimal_fields(log_read_client: HttpTestHelper, log_write_client, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
    ...
    resp = await log_read_client.assert_get(f"/repos/{repo.id}/logs/{log.id}/attachments/0")
    assert resp.headers["Content-Disposition"] == "attachment; filename=file.txt"
    assert resp.headers["Content-Type"] == "text/plain; charset=utf-8"
    assert resp.content == b"test data"


async def test_get_log_attachment_binary_and_all_fields(log_read_client: HttpTestHelper, log_write_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
...
async def test_get_log_attachment_not_found_log_id(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await log_read_client.assert_get(
        f"/repos/{repo.id}/logs/{UNKNOWN_OBJECT_ID}/attachments/0", expected_status_code=404
    )


async def test_get_log_attachment_not_found_attachment_idx(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_rw_client)
    await log_rw_client.assert_get(f"/repos/{repo.id}/logs/{log.id}/attachments/0", expected_status_code=404)


async def test_get_log_attachment_forbidden(log_write_client: HttpTestHelper, no_permission_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_write_client)
...
async def test_get_logs(log_write_client: HttpTestHelper, log_read_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_get_logs_unknown_repo(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await log_read_client.assert_get(f"/repos/{UNKNOWN_OBJECT_ID}/logs", expected_status_code=404)


async def test_get_logs_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_get_logs_limit_and_cursor(log_rw_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_get_logs_filter_event_name(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
...
async def test_get_logs_filter_event_category(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["event"]["category"] = "find_me"

    await _test_get_logs_filter(log_rw_client, repo, func, {"event_category": "find_me"})


async def test_get_logs_filter_actor_type(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["actor"] = {
            "type": "find_me",
            "id": "user:123",
...
async def test_get_logs_filter_actor_name(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["actor"] = {
            "type": "user",
            "id": "user:123",
...
async def test_get_logs_filter_resource_type(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["resource"] = {
            "type": "find_me",
            "id": "core",
...
async def test_get_logs_filter_resource_name(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["resource"] = {
            "type": "module",
            "id": "core",
...
async def test_get_logs_filter_tag_category(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["tags"] = [
            {"id": "simple_tag"},
...
async def test_get_logs_filter_tag_name(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["tags"] = [
            {"id": "simple_tag"},
...
async def test_get_logs_filter_tag_id(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["tags"] = [
            {"id": "simple_tag"},
...
async def test_get_logs_filter_node_id_exact_node(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["node_path"] = [
...
async def test_get_logs_filter_node_id_ascendant_node(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    def func(log):
        log["node_path"] = [
            {"id": "find_me:1", "name": "Customer 1"},
            {"id": "find_me:2", "name": "Customer 2"}
...
async def test_get_logs_filter_since(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log1 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-01T00:00:00Z"))
    log2 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-02T00:00:00Z"))
...
async def test_get_logs_filter_until_milliseconds(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log1 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2023-12-31T23:59:59.500Z"))
    log2 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-01T00:00:00Z"))
...
async def test_get_logs_filter_between_since_and_until(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log1 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-01T00:00:00Z"))
    log2 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-02T00:00:00Z"))
    log3 = await repo.create_log(log_rw_client, saved_at=datetime.fromisoformat("2024-01-03T00:00:00Z"))
...
async def test_get_logs_filter_multiple_criteria(log_rw_client: HttpTestHelper, repo: PreparedRepo):
...
async def test_get_logs_empty_string_filter_params(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_rw_client)

    resp = await log_rw_client.assert_get(f"/repos/{repo.id}/logs", params={
...
async def test_get_logs_filter_no_result(log_rw_client: HttpTestHelper, repo: PreparedRepo):
    log = await repo.create_log(log_rw_client)
...
async def test_get_log_event_categories(log_read_client: HttpTestHelper, repo: PreparedRepo):
    ...
    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/event-categories", [f"category_{i}" for i in range(5)]
    )


async def test_get_log_event_categories_unknown_repo(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await log_read_client.assert_get(
        f"/repos/{UNKNOWN_OBJECT_ID}/logs/event-categories", expected_status_code=404
    )


async def test_get_log_event_categories_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
        f"/repos/{repo.id}/logs/event-categories"
    )


async def test_get_log_event_categories_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/event-categories")
...
async def test_get_log_event_names(log_read_client: HttpTestHelper, repo: PreparedRepo):
    ...
    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/events", [f"name_{i}" for i in range(5)]
    )
...
async def test_get_log_event_names_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/events")


async def test_get_log_event_names_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
...
async def test_get_log_actor_types(log_read_client: HttpTestHelper, repo: PreparedRepo):
    ...
    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/actor-types", [f"type_{i}" for i in range(5)]
    )


async def test_get_log_actor_types_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/actor-types")


async def test_get_log_actor_types_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
        f"/repos/{repo.id}/logs/actor-types"
    )


async def test_get_log_resource_types(log_read_client: HttpTestHelper, repo: PreparedRepo):
    for i in reversed(range(5)):  # insert in reverse order to test sorting
        await consolidate_log_resource(repo.db, Log.Resource(type=f"type_{i}", id=f"id_{i}", name=f"name_{i}"))

    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/resource-types", [f"type_{i}" for i in range(5)]
    )


async def test_get_log_resource_types_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/resource-types")


async def test_get_log_resource_types_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
        f"/repos/{repo.id}/logs/resource-types"
    )


async def test_get_log_tag_categories(log_read_client: HttpTestHelper, repo: PreparedRepo):
    ...
    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/tag-categories", [f"category_{i}" for i in range(5)]
    )


async def test_get_log_tag_categories_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/tag-categories")


async def test_get_log_tag_categories_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
        f"/repos/{repo.id}/logs/tag-categories"
    )


async def test_get_log_nodes_without_filters(log_read_client: HttpTestHelper, repo: PreparedRepo):
    for i in range(4):
...
async def test_get_log_nodes_with_filters(log_read_client: HttpTestHelper, repo: PreparedRepo):
    for i in range(5):
        for j in "a", "b", "c", "d", "e":
            await consolidate_log_node_path(
                repo.db, [
    ...
    await do_test_page_pagination_common_scenarios(
        log_read_client, f"/repos/{repo.id}/logs/nodes?parent_node_id=customer:2",
...
async def test_get_log_nodes_empty(log_read_client: HttpTestHelper, repo: PreparedRepo):
    await do_test_page_pagination_empty_data(log_read_client, f"/repos/{repo.id}/logs/nodes")


async def test_get_log_nodes_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
...
async def test_get_log_node_forbidden(no_permission_client: HttpTestHelper, repo: PreparedRepo):
    await no_permission_client.assert_forbidden_get(
