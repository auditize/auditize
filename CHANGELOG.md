...
- Add a global fulltext search field
- Use fulltext search for search on plain text fields (i.e. custom field values)
- Use asciifolding text analysis in Elasticsearch to make search accent-insensitive
- OpenAPI documentation improvements
- Add `POST /api/repos/{repo_id}/logs/import` endpoint (that allows setting the `id` and `saved_at` fields)
- the API now returns datetimes with milliseconds precision
- Upgrade back & front dependencies (among which Mantine 7.x => 8.3, React 18 => 19.2, React Router 6.x => 7.2)
- Fix non-working access token authentication when the token has been generated by a non-superadmin API key (used by the web component)
- Fix log entities not properly scoped by repository on read operations
